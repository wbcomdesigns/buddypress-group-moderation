...
if ( ! class_exists( 'Wbcom_Admin_Settings' ) ) {
	...
	class Wbcom_Admin_Settings {
		...
		public function wbcom_enqueue_admin_scripts() {
			...
			if ( ! wp_script_is( 'wbcom_admin_setting_js', 'enqueued' ) ) {
				...
				wp_register_script(
					...
					BP_GROUP_MODERATION_PLUGIN_URL . 'admin/wbcom/assets/js/wbcom-admin-setting' . $js_extension,
					array( 'jquery' ),
					time(),
					false
				);
				wp_localize_script(
					...
					array(
						...
						'activate_text'   => esc_html__( 'Activate', 'buddypress-group-moderation' ),
						'deactivate_text' => esc_html__( 'Deactivate', 'buddypress-group-moderation' ),
			...
			if ( ! wp_style_is( 'wbcom-admin-setting-css', 'enqueued' ) ) {
				...
				wp_enqueue_style( 'wbcom-admin-setting-css', BP_GROUP_MODERATION_PLUGIN_URL . 'admin/wbcom/assets/css' . $rtl_css . '/wbcom-admin-setting' . $css_extension );
			}
		}
		...
		public function wbcom_admin_additional_pages() {
			add_submenu_page(
				'wbcomplugins',
				esc_html__( 'Our Plugins', 'buddypress-group-moderation' ),
				esc_html__( 'Our Plugins', 'buddypress-group-moderation' ),
				'manage_options',
				'wbcom-plugins-page',
			...
			add_submenu_page(
				'wbcomplugins',
				esc_html__( 'Our Themes', 'buddypress-group-moderation' ),
				esc_html__( 'Our Themes', 'buddypress-group-moderation' ),
				'manage_options',
				'wbcom-themes-page',
			...
			add_submenu_page(
				'wbcomplugins',
				esc_html__( 'Support', 'buddypress-group-moderation' ),
				esc_html__( 'Support', 'buddypress-group-moderation' ),
				'manage_options',
				'wbcom-support-page',
					...
					<ul>
						<li class="wb_admin_nav_item <?php echo esc_attr( $settings_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcomplugins'; ?>" id="wb_admin_nav_trigger_settings">
								...
								<h4><?php esc_html_e( 'Settings', 'buddypress-group-moderation' ); ?></h4>
							</a>
						</li>
						<li class="wb_admin_nav_item <?php echo esc_attr( $plugin_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcom-plugins-page'; ?>" id="wb_admin_nav_trigger_extensions">
								...
								<h4><?php esc_html_e( 'Themes & Extension', 'buddypress-group-moderation' ); ?></h4>
							</a>
						</li>
						<li class="wb_admin_nav_item <?php echo esc_attr( $support_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcom-support-page'; ?>" id="wb_admin_nav_trigger_support">
								...
								<h4><?php esc_html_e( 'Help & Support', 'buddypress-group-moderation' ); ?></h4>
							</a>
						</li>
		...
		/**
		 * Hide all admin notices from the setting page.
		 *
		 * @return void
		 */
		public function wbcom_hide_all_admin_notices_from_setting_page() {
			$wbcom_pages_array  = array( 'wbcomplugins', 'wbcom-plugins-page', 'wbcom-support-page', 'bp-group-moderation');
